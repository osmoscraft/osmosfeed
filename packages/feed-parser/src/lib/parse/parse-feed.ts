...
import type { JsonFeedChannel, JsonFeedChannelMetadata, JsonFeedItem } from "../json-feed";

export interface XmlFeedParser {
  isMatch: (root: Cheerio<Document>) => boolean;
  selectChannel: (root: Cheerio<Document>) => Cheerio<Element>;
  resolveChannel: (channelElement: Cheerio<Element>) => JsonFeedChannelMetadata;
...
export function parseFeed(input: ParseFeedInput): JsonFeedChannel {

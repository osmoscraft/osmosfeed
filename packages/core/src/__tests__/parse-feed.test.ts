...
describe("Parse feed", ({ spec }) => {
  ...
  spec("No error thrown on basic setup", async () => {
    await runMatrix(["empty-atom-v1.xml", "empty-rss-v2.xml", "empty-rdf-v1.xml"], async (filename) => {
  ...
  spec("Parse 1 item/RSS 2.0", async () => {
    const jsonFeed = await parseXmlFixture("single-item-rss-v2.xml");
    await expect(jsonFeed.items.length).toEqual(1);
  });

  spec("Parse multiple items/RSS 2.0", async () => {
    const jsonFeed = await parseXmlFixture("multi-item-rss-v2.xml");
  ...
  spec("Parse item summary of plaintext/RSS 2.0", async () => {
    const jsonFeed = await parseXmlFixture("item-summary-rss-v2.xml");
  ...
  spec("Parse item with description but no content:encoded/RSS 2.0", async () => {
    const jsonFeed = await parseXmlFixture("item-description-rss-v2.xml");

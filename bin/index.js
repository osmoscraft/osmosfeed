...
const htmlparse2 = __importStar(__webpack_require__(2928));
const axios_1 = __importDefault(__webpack_require__(6545));
async function run() {
    const sourcesText = fs_1.default.readFileSync(path_1.default.resolve("sources.yaml"), "utf8");
    /** @type {string[]} */
    const sources = js_yaml_1.default.safeLoad(sourcesText);
    const pagesAsync = sources.map(async (source) => {
        var _a;
        const response = await axios_1.default.get(source.href);
        const xmlString = response.data;
        const feed = htmlparse2.parseFeed(xmlString);
        const items = (_a = feed.items) !== null && _a !== void 0 ? _a : [];
        const now = Date.now();
   ...
   <h3>${date}</h3>
   ${posts
        .map((post) => `<article><a href="${post.link}">${post.title}</a></article>`)
        .join("\n")}
  `)
        .join("\n");
    fs_1.default.mkdirSync(path_1.default.resolve("dist"), { recursive: true });
    fs_1.default.writeFileSync(path_1.default.resolve("dist/index.html"), outputHtml);
}
run();
